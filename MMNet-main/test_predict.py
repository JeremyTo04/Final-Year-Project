...
def run_training():
    ...
    LOSO = [
        'S59', 'S60', 'S61', 'S63', 'S64', 'S65', 'S08', 'S09', 'S10',
        'S12', 'S13_1', 'S13_2', 'S14', 'S16', 'S17', 'S18', 'S20',
        'S21', 'S22', 'S24', 'S25', 'S27', 'S30_1', 'S30_2', 'S31',
        'S33', 'S34', 'S36', 'S37', 'S38', 'S41', 'S42', 'S45',
        'S46', 'S49', 'S50_1', 'S51', 'S52_2', 'S53', 'S55', 'S56',
        'S57', 'S58_1'
    ]
    ...
    for subj in LOSO:
        ...
        train_dataset = RafDataSet(
            args.raf_path, phase='train', num_loso=subj, transform=data_transforms,
            basic_aug=True, transform_norm=data_transforms_norm
        )
        if len(train_dataset) == 0:
            raise ValueError("The training dataset is empty. Please check the dataset paths and filtering logic.")
        ...
        if args.optimizer == 'adam':
            ...
            optimizer_all = torch.optim.SGD(params_all, lr=args.lr,
                                            momentum=args.momentum,
                                            weight_decay=1e-4)
        else:
            raise ValueError("Optimizer not supported.")
        ...
        for epoch in range(1, 2):  # Reduced to 10 epochs for demonstration
            ...
            for batch_i, (
                image_on0, image_on1, image_on2, image_on3, image_apex0,
                image_off0, image_off1, image_off2, image_off3,
                label_all, f, label_au
            ) in enumerate(train_loader):
                batch_sz = image_on0.size(0)
                
                # Move data to device
                images = [
                    image_on0, image_on1, image_on2, image_on3,
                    image_apex0, image_off0, image_off1, image_off2, image_off3
                ]
                ...
                correct_sum += torch.eq(preds, label_all).sum().item()
                iter_cnt += 1
            ...
            print(f'[Epoch {epoch}] Training Accuracy: {acc:.4f}, Loss: {avg_loss:.3f}')
            ...
            correct_val = 0
            iter_val = 0
            all_data = []  # Reset all_data for current subject
            ...
            with torch.no_grad():
                for batch_i, (
                    ...
                    val_loss += loss.item()
                    _, preds = torch.max(outputs, 1)
                    correct_val += torch.eq(preds, label_all).sum().item()
                    iter_val += 1
                    
                    # Store predictions and labels
                    for j in range(preds.size(0)):
                        all_data.append([f[j], preds[j].item(), label_all[j].item()])
                    
                    # Calculate per-class metrics
                    for cls in range(3):
                        pos_pred[cls] += torch.sum(preds == cls).item()
                        pos_label[cls] += torch.sum(label_all == cls).item()
                        TP[cls] += torch.sum((preds == cls) & (label_all == cls)).item()
            ...
            avg_val_loss = val_loss / iter_val
            val_acc = correct_val / len(val_dataset)
            print(f'[Epoch {epoch}] Validation Accuracy: {val_acc:.4f}, Loss: {avg_val_loss:.3f}, F1-score: {AVG_F1:.3f}')
            
            # Update maximum metrics
            if correct_val > max_corr:
                max_corr = correct_val
            if AVG_F1 > max_f1:
                max_f1 = AVG_F1
                max_pos_label = pos_label
                max_pos_pred = pos_pred
        ...
        num_sum += max_corr
        pos_label_ALL += max_pos_label
        pos_pred_ALL += max_pos_pred
        TP_ALL += max_TP
        val_now += len(val_dataset)
        
        # Final metrics for the subject
        SUM_F1_ALL = 0
        count_all = 0
        for index in range(3):
            if pos_label_ALL[index] != 0 or pos_pred_ALL[index] != 0:
                SUM_F1_ALL += 2 * TP_ALL[index] / (pos_pred_ALL[index] + pos_label_ALL[index])
                count_all += 1
        F1_ALL = SUM_F1_ALL / count_all if count_all > 0 else 0
        
        print(f"[..........{subj}] Correct Predictions: {max_corr}, Total Validation Samples: {len(val_dataset)}")
        print(f"[ALL_corr]: {num_sum}, [ALL_val]: {val_now}")
        print(f"[F1_now]: {max_f1:.4f}, [F1_ALL]: {F1_ALL:.4f}")
        ...
        # **Print Predicted Emotion, Subject, and Actual Label**
        print(f"\n=== Predictions for Subject: {subj} ===")
        print(df.to_string(index=False))
        print(f"=== End of Predictions for Subject: {subj} ===\n")
